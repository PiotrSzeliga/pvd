...
1. Open the notebook my-first-notebook.ipynb. Then read the contents of the notebook and complete all tasks.

1. Create your first notebook called bmi.ipynb in which you will add the ability to calculate **Body Mass Index (BMI)**. Detailed information on this topic can be found on Wikipedia. 
   1. Add a text cell in your notebook where you put the title (Body mass Index).
   1. In the next text cell add a short explanation. You can copy and format the text from Wikipedia.
   1. Below the description, add a code cell where you can enter the person's height and weight from the keyboard.
   1. Below the calculation cell, add a text cell with the chart showing body mass index (BMI) for a range of heights and weights:
      <https://upload.wikimedia.org/wikipedia/commons/c/cc/BMI_chart.png> 
   1. Then, calculate the BMI (Body Mass Index). Enter your height and weight from the keyboard. Display height, weight and calculated BMI.
   1. Finally, print preview your notebook.
...
1. Pandas Data Model
    ...
    ![Pandas Data Model](https://cdn-images-1.medium.com/v2/resize:fit:2104/1*2rTTJJ0IhXuDB-GwVz2HxA.png)
...
1. Kraków had a recorded population of 774,839 in 2019, which increased to 804,237 in 2023. Detailed information can be found at:
    ...
    Create a Jupyter Notebook named krakow.ipynb. Add text cells with a title, a short description and an image about Krakow. You can copy the text from Wikipedia. Then, in the notebook, create a Series containing information about the population of Krakow in the years 1985 to 2023. Calculate and display:
